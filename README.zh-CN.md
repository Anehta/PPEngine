...
![Version](https://img.shields.io/badge/%E7%89%88%E6%9C%AC-1.9.8-blue?style=flat-square)
...
| 📦 压缩包 | 📝 内容 |
| :-- | :-- |
| 🎨 `PPEngineV1.9.8.7z` | PPEngine 查看器 **+ 操作手册** |
...
| | 产品 | 是什么 |
| :-- | :-- | :-- |
| 🚢 | **PPEngine** —— *专业模型评审器* | 聚焦的免费桌面 + 网页应用，负责大规模工业模型的轻量化转换、查看、数字化交付与**设计评审**。当前支持 **`REV` · `RVM` · `STEP` · `VUE`（SmartPlant 3D）· `3D 高斯溅射`** 格式。专为**船舶建造、海洋工程与工厂设计**打造。 |
...
### 🏭 工厂设计 — SmartPlant 3D `.vue` 原生支持 &nbsp;<sub>**NEW · v1.9.8**</sub>

> **可直接导入 `.vue` 文件**，自研解析器，**无转换层**。
> 几何与 BRep 的解析及构建全部由自研 **PP3D Core 几何内核**完成 —— **彻底摒弃任何外部几何内核依赖**。
> 工厂级 PLM 设备库现可**在浏览器中直接加载**。

<div align="center">

<img src="capture/vue.png" alt="SmartPlant 3D .vue 工厂模型在 PPEngine 中加载" width="88%" />

</div>

**✨ 价值点：**

- 🔧 **PP3D Core 自研几何内核** —— 自有 CAD 几何层，不依赖任何第三方内核。为未来的实时几何编辑与参数化建模铺好底层。
- ⚡ **自研 `.vue` 解析器** —— 无中间格式、无二次转换，完整保真。
- 🌐 **Web 可用** —— 工厂模型可在浏览器中以原生级画质实时渲染。
- 🏗️ 专为**流程工厂 · 油气 · 电力**而生 —— 法兰、支吊架、电缆桥架，每一处细节都看得到。
...
- 🌐 **全平台通跑 —— 包括浏览器。** 桌面端（DX12 / Vulkan）· **Web（WebGPU）** · 头显（OpenXR）。Splat 场景**在浏览器里就能完整渲染**，不装插件、不用下载、一个链接搞定。
...
> 完整操作手册**随发布包一起提供** —— 解压 `PPEngineV1.9.8.7z` 后即可打开里面的手册，查阅详细用法、快捷键与故障排查。
